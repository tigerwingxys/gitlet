...
class Commit implements Serializable {
	...
	HashMap<String,String> getChildCommits() {
		return this.childBranchCommits;
	}
	...
	void log() {
		...
		Utils.message("Date: %s", timestamp);
		Utils.message(message);
	}

	static File[] exists(String commitID) {
		File[] files = Config.COMMIT_FOLDER.listFiles(new FileFilter() {
			
			@Override
			public boolean accept(File pathname) {
				return pathname.getName().startsWith(commitID);
			}
		});
		return files;
	}
	...
	boolean hasParent(String parentID) {
		Commit aCommit = this;
		while(true) {
			String aID = aCommit.getParentCommitID();
			if (aID.equals(parentID)) {
				return true;
			}
			if (aID.equals(Config.INIT_COMMIT_ID)) {
				break;
			}
			aCommit = Commit.load(aID);
		}
		return false;
	}
	...
	static Commit getSplitCommit(Commit currHead, Commit otherHead) {
		...
		if (currBranch.equals(otherBranch)) {
			return currHead.hasParent(otherHead.commitID) ? otherHead : currHead;
		}
